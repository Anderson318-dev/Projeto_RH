...
df_t_ano['turnover_taxa'] = (((df_t_ano['admissões'] + df_t_ano['desligamentos']) / 2) / df_t_ano['colaboradores'].replace(0, 1)) * 100
...
# --- GRÁFICO DE EVOLUÇÃO (MOVIMENTAÇÃO VS EFETIVO COM RÓTULOS) ---
...
# Linha de Efetivo Total (Substituindo Turnover)
...
fig_evol.update_layout(
    yaxis=dict(title="Movimentação (Adm/Des)", showgrid=False),
...
with col_left:
    ...
    # Informação de Afastados transferida para cá
    qtd_afastados = len(df_af_mes)
    st.info(f"**Total de Afastados no período:** {qtd_afastados} colaborador(es)")
    ...
    if not df_af_mes.empty:
        ...
        st.write("Sem afastamentos para os filtros selecionados.")

with col_right:
    st.subheader("🏢 Colaboradores por Empresa")
    # Gráfico de pizza com quantidade e percentual
